 ...
 */
// based off https://github.com/apache/beam/blob/7bfeca2a6ceb4ef8ac3c9822a59d770490c1e6f9/sdks/java/core/src/main/java/org/apache/beam/sdk/io/Read.java
...
public class ReadFix {
  ...
  static class UnboundedSourceAsSDFWrapperFn<OutputT, CheckpointT extends CheckpointMark>
    ...
    // this is made static to ensure that cleanUp() is called regularly and readers will always be re-used
    private static Cache<Object, UnboundedReader> cachedReaders = null;
            ...
            CacheBuilder.newBuilder()
                ...
                .removalListener(
                    (RemovalListener<Object, UnboundedReader>)
                        removalNotification -> {
                          // handle all cases except for explicit invalidation
                          // wasEvicted did not handle replacements
                          // this ensures no readers are removed without being destroyed
                          // or taken back by a 
    ...
    abstract static class UnboundedSourceValue<T> {
      public static <T> UnboundedSourceValue<T> create(
        ...
        return new AutoValue_ReadFix_UnboundedSourceAsSDFWrapperFn_UnboundedSourceValue<T>(
    ...
    abstract static class UnboundedSourceRestriction<OutputT, CheckpointT extends CheckpointMark>
      ...
      public static <OutputT, CheckpointT extends CheckpointMark>
          UnboundedSourceRestriction<OutputT, CheckpointT> create(
              ...
              Instant watermark) {
        return new AutoValue_ReadFix_UnboundedSourceAsSDFWrapperFn_UnboundedSourceRestriction<>(
    ...
    private static class UnboundedSourceAsSDFRestrictionTracker<
      ...
      private void initializeCurrentReader() throws IOException {
        ...
        if (currentReader == null) {
          ...
          // readerHasBeenStarted is not set here because getProgress might have put it into the cache without starting
          // this assumes that reader can be start()-ed multiple times which has been done for UnboundedSolaceReader
