  ...
  PCollection<SolaceTextRecord> input =
    pipeline
      .apply(SolaceIO.<SolaceTextRecord>readMessage()
        ...
        .withSenderMessageId(boolean useSenderMi))
        .withForceActive(boolean useForceActive))
...
| SenderMessageId | false            | Use Sender Sequence Number else JCSMP autogenerated messageId |
| timeoutInMillis | 100              | Time between checkpoints|
| forceActive     | false            | Force new Readers to take activity on Solace queue, must set adminUsername and adminPassword if tue | 
| adminUsername   | null             | Admin Username to disconnect old queue bindings |
| adminPassword   | null             | Admin Password to disconnect old queue bindings |
...
## Asserting activity on the Solace queue

To maintain order the Solace queues are in exclusive access mode.  If the UnboundedSolaceReader binds to a queue that is previously bound, it will become a secondary bind and not recieve messages until the primary bind disconnects.  If you use the forceActive flag with adminUsername and adminPassword set, the UnboundedSolaceReader will detect if it is active on the queue, and disconnect other client if needed to become active.

It is recommended to use a specific administrator that only has read/write access on messageVPN in which the queues exist, not the default administrator account.  See [here](https://docs.solace.com/Configuring-and-Managing/Configuring-Internal-CLI-User-Accounts.htm) for info on how to create administrator accounts.
